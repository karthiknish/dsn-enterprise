              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {topPages.length > 0 ? (
                  topPages.map((row) => {
                    ...
                    const totalViews = topPages.reduce((acc, r) => acc + parseInt(r.metricValues[0].value, 10), 0);
                    const percentage = ((views / totalViews) * 100).toFixed(1);

                    return (
                      <tr key={row.dimensionValues[0].value} className="hover:bg-gray-50 transition-colors">
                          ...
                          <div className="flex flex-col items-end">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <div className="w-16 bg-gray-100 rounded-full h-1">
                                <div className="bg-accent h-1 rounded-full" style={{ width: `${percentage}%` }} />
                              </div>
  ...
  return (
    <div className="w-full overflow-x-auto scrollbar-hide">
      <div className="min-w-[600px]">
        <svg aria-hidden="true" viewBox={`0 0 ${width} ${height}`} className="w-full h-auto drop-shadow-sm">
          {/* Grid Lines */}
