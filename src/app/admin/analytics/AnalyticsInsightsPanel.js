export default function AnalyticsInsightsPanel({ bounceRateRaw }) {
	const hasBounce =
		bounceRateRaw !== null &&
		bounceRateRaw !== undefined &&
		bounceRateRaw !== "" &&
		Number.isFinite(parseFloat(bounceRateRaw));
	const bouncePercent = hasBounce
		...
		<div className="space-y-6">
			<div className="bg-gradient-to-br from-accent to-accent-700 rounded-2xl shadow-lg p-6 text-white">
				...
				<p className="text-accent-50 opacity-90 text-sm leading-relaxed">
					{bouncePercent === null
						? "Bounce rate is unavailable for this period. Once Google Analytics reports data, this panel will show it."
						: `Your bounce rate is currently ${bouncePercent}%. To improve engagement, try adding more internal links in your blog posts and clarifying call-to-actions on product pages.`}
				</p>
				{/* Previously this always read "GA4 Live" and "Last calculated just
				    now", which asserted a working connection even when GA4 had
				    returned nothing at all. */}
				<div className="mt-4 pt-4 border-t border-accent-500/30 flex items-center justify-between text-xs font-medium">
					<span>{bouncePercent === null ? "Awaiting data" : "From GA4"}</span>
					<span className="px-2 py-1 bg-accent-500/40 rounded-full">
						{bouncePercent === null ? "No data" : "GA4"}
					</span>
				</div>
			</div>
				...
				<div className="space-y-4">
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-xs text-gray-500">
								Track Sessions vs Users. A higher ratio means users are
								returning to your site frequently.
							</p>
						</div>
					</div>
				</div>
