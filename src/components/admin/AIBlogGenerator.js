...
export default function AIBlogGenerator({
	...
	const improveContent = async (instruction) => {
		if (!generatedContent.trim()) {
			setError("Generate content first before improving");
			return;
		}
		if (!instruction.trim()) {
			setError("Enter an improvement instruction");
			return;
		}
		...
		try {
			...
			if (!response.ok) {
				setError(data.error || "Failed to improve content");
				return;
			}
				...
				<div className="mt-6">
					...
					<div className="mt-4 space-y-3">
						...
						<div className="flex gap-2">
							...
							<button
								type="button"
								onClick={() => improveContent(improveInstruction)}
